"""
BatteryPopup — PIL-rendered hover info popup with rounded corners, graph,
settings page, rows config page, and per-app wattage page.
"""
import math
import os
import threading
...
class BatteryPopup:
    """Hover info popup — multi-page PIL-rendered with true rounded corners."""

    _TC     = "#020202"   # transparent key
    _TC_RGB = (2, 2, 2)
    _MIN_W  = 280
    ...
    def __init__(self, root, wx, wy, ww, wh, bat, label, secs,
        ...
        self._sessions      = sessions   # completed sessions list

        # Page state
        self.page           = "dashboard"   # dashboard | settings | rows_config | apps
        self._graph_index   = -1            # -1 = live current session

        # Apps page state
        self._process_tracker = bat_mod.ProcessTracker()
        self._app_list: list  = []
        self._app_scroll      = 0          # scroll offset (rows)
        ...
        self._hov  = _c["hover"]
        self._acc  = (0, 120, 212) if dark else (0, 99, 177)  # accent blue
        ...
        self._position_window(wx, wy, ww, wh)
        self.win.update()
    ...
    def _compute_height(self, pw, s):
        """Compute popup height for current page."""
        if self.page == "dashboard":
            ...
            # 5 stepper rows with (RH+4) each + customize_rows row + move_icon row (both RH)
            ph = int((self._PY + self._TH + self._SH
                      + 5 * (self._RH + 4)
                      + 2 * self._RH
                      + self._SH + self._QH + self._PY) * s)
            ...
            n_rows = len(config.ROWS_CONFIG)
            ph = int((self._PY + self._TH + self._SH
                      + n_rows * self._RH
                      + self._SH + self._QH + self._PY) * s)
            return ph
        elif self.page == "apps":
            # 1 header row + 8 visible app rows + optional scroll indicator
            n_rows = 10
            ph = int((self._PY + self._TH + self._SH
                      + n_rows * self._RH
                      + self._SH + self._QH + self._PY) * s)
    ...
    def _position_window(self, wx, wy, ww, wh):
    ...
    def _redraw(self):
        """Re-render the current page and update the canvas image."""
        s  = self._s
        pw = self._pw
        ...
        if ph != self._ph:
            ...
            self._position_window(self._wx, self._wy, self._ww, self._wh)
            self.win.geometry(f"{pw}x{ph}+"
    ...
    def _render(self, w, h, s):
        ...
        if self.page == "dashboard":
            ...
            d.text((px, y + int(self._TH * s) // 2), "Settings",
                   font=tfnt, fill=a(self._fg), anchor="lm")
        elif self.page == "rows_config":
            d.text((px, y + int(self._TH * s) // 2), "Customize Rows",
                   font=tfnt, fill=a(self._fg), anchor="lm")
        elif self.page == "apps":
            d.text((px, y + int(self._TH * s) // 2), "App Battery Usage",
                   font=tfnt, fill=a(self._fg), anchor="lm")
        y += int(self._TH * s)
    ...
    def _rows(self):
        ...
        _data = {
            ...
            "screen_on":   (self._IC["screen"],   "Screen On",   "\u2014"),
    ...
    def _draw_graph(self, d, img, px, y, w, s, a):
        """Draw the battery-history area chart with session nav arrows."""
        gh    = int(self._GH       * s)
        ...
        # Navigation arrows — drawn in top-right of graph box
        arrow_r = int(config.POPUP_ICON_SIZE * s)
        ...
        # Session label in top-left of graph
        if self._graph_index != -1 and self._sessions:
            sess   = self._sessions[self._graph_index]
            s_type = sess.get("type", "")
            s_lbl  = f"{'Charging' if s_type == 'charging' else 'Discharging'} Session"
            lfnt2  = load_font(max(self._G_FONT, int(self._G_FONT * s)))
            d.text((cx0 + int(6 * s), cy0 + int(4 * s)), s_lbl,
                   font=lfnt2, fill=a(self._acc), anchor="lt")
        elif self._graph_index == -1:
            lfnt2 = load_font(max(self._G_FONT, int(self._G_FONT * s)))
            d.text((cx0 + int(6 * s), cy0 + int(4 * s)), "Live",
                   font=lfnt2, fill=a(self._acc), anchor="lt")

        # Nav arrows
        can_go_left  = len(self._sessions) > 0 and self._graph_index > 0 or (self._graph_index == -1 and len(self._sessions) > 0)
        can_go_right = self._graph_index != -1

        l_col = a(self._fg)   if can_go_left  else a(self._fg2)
        r_col = a(self._acc)  if can_go_right else a(self._fg2)

        nav_mid_y = nav_y + arrow_r // 2
        d.text(((nav_lx0 + nav_lx1) // 2, nav_mid_y), self._IC["chevL"],
               font=ifnt_sm, fill=l_col, anchor="mm")
        d.text(((nav_rx0 + nav_rx1) // 2, nav_mid_y), self._IC["chevR"],
               font=ifnt_sm, fill=r_col, anchor="mm")

    # ── Settings page ─────────────────────────────────────────────────────────

    def _render_settings(self, d, px, y, w, s, a, ifnt, nfnt, sfnt):
        """Draw settings rows with stepper controls and reset icons."""
        rh = int((self._RH + 4) * s)
        ix = int(11 * s)
        ...
        for key, label, unit, mn, mx, step in settings_rows:
            ...
            total_w = btn_w + gap + val_tw + gap + btn_w + gap + int(20 * s)  # +reset
            x0 = w - px - total_w
            ...
            # [+]
            p_x0, p_x1 = m_x1 + gap + val_tw + gap, m_x1 + gap + val_tw + gap + btn_w
        ...
        d.text((px + ix, my2), self._IC["move"],
               font=ifnt, fill=a(self._icol if not hov_mv else self._fg), anchor="mm")
    ...
    def _render_rows_config(self, d, px, y, w, s, a, ifnt, nfnt):
        ...
        for i, row in enumerate(config.ROWS_CONFIG):
            ...
            chk_x   = px + ix
            d.text((chk_x, my), chk_ic, font=ifnt, fill=chk_col, anchor="mm")

            # Row label
            d.text((chk_x + int(18 * s), my), lbl,
            ...
            dn_col = a(self._fg if hov_dn else self._icol) if i < len(config.ROWS_CONFIG) - 1 else a(self._fg2)

            d.text(((up_x0 + up_x1) // 2, my), self._IC["arrow_up"],  font=ifnt, fill=up_col, anchor="mm")
            d.text(((dn_x0 + dn_x1) // 2, my), self._IC["arrow_dn"],  font=ifnt, fill=dn_col, anchor="mm")
    ...
    def _render_apps(self, d, px, y, w, s, a, ifnt, nfnt):
        """Render live per-process wattage list."""
        rh = int(self._RH * s)
        self._apps_hit_regions = {}

        # Header row
        hdr_y = y + rh // 2
        d.text((px + int(26 * s), hdr_y), "Process",
               font=nfnt, fill=a(self._fg2), anchor="lm")
        d.text((w - px - int(22 * s), hdr_y), "Watts",
               font=nfnt, fill=a(self._fg2), anchor="rm")
        y += rh
        ...
        for idx, proc in enumerate(items[start:end]):
            ...
            # Kill button
            kill_x0 = w - px - int(20 * s)
            kill_x1 = w - px
            hov_kill = self._hover_key == ("kill", start + idx)
            d.text(((kill_x0 + kill_x1) // 2, my), self._IC["skull"],
                   font=ifnt,
                   fill=a(self._red if hov_kill else (self._icol if hov_row else self._fg2)),
                   anchor="mm")

            d.text((px + int(8 * s), my), name,
                   font=nfnt, fill=a(self._fg), anchor="lm")
    ...
    def _render_bottom_bar(self, d, px, y, w, s, a, ifnt):
        """Render bottom icon bar; changes based on current page."""
        b     = int(self._QH * s)
        bar_y = y + b // 2
        gap   = int(6 * s)
        ...
        for btn_key, glyph, is_quit in buttons:
    ...
    def _hit_graph_nav(self, x, y):
        if not hasattr(self, "_graph_nav"):
            return None
    ...
    def _on_click(self, event):
        ...
        if btn == "back":
            ...
            self._redraw()
            return
        if btn == "github" and self.page == "dashboard":
            import webbrowser
            webbrowser.open("https://github.com/AhmarZaidi/wincity")
            return

        # Graph nav arrows (dashboard page)
        if self.page == "dashboard":
            nav = self._hit_graph_nav(ex, ey)
            if nav == "left":
                if self._graph_index == -1:
                    # Go to last saved session
                    if self._sessions:
        ...
        if self.page == "rows_config":
            self._handle_rows_config_click(ex, ey)
            return

        # Apps page
        if self.page == "apps":
            self._handle_apps_click(ex, ey)
            return

    def _handle_settings_click(self, x, y):
        if not hasattr(self, "_settings_hit_regions"):
            return
        settings_meta = {
    ...
    def _handle_rows_config_click(self, x, y):
        if not hasattr(self, "_rows_hit_regions"):
            return
    ...
    def _handle_apps_click(self, x, y):
        if not hasattr(self, "_apps_hit_regions"):
            return
    ...
    def _on_motion(self, event):
        ...
        if self.page == "settings" and hasattr(self, "_settings_hit_regions"):
            for key, regions in self._settings_hit_regions.items():
        ...
        if self.page == "rows_config" and hasattr(self, "_rows_hit_regions"):
            for i, regions in self._rows_hit_regions.items():
                for action in ("toggle", "up", "down"):
                    rx0, ry0, rx1, ry1 = regions[action]
                    if rx0 <= ex < rx1 and ry0 <= ey < ry1:
        ...
        if self.page == "apps" and hasattr(self, "_apps_hit_regions"):
            for idx, regions in self._apps_hit_regions.items():
                kx0, ky0, kx1, ky1 = regions["kill"]
        ...
        is_btn = isinstance(new_key, str) and new_key in ("settings", "apps", "quit", "back",
                                                            "github", "customize_rows", "move_icon")
    ...
    def _on_scroll(self, event):
        if self.page == "apps":
            ...
            max_scroll = max(0, n - 8)
            self._app_scroll = max(0, min(max_scroll, self._app_scroll + delta))
            self._redraw()
    ...
    def push_update(self, bat, label, secs, rate_mw, designed_mwh, full_mwh,
        ...
        """Called by widget's popup_refresh_tick to push live data into the popup."""
        self._bat           = bat
        self._label         = label
        self._secs          = secs
        self._rate_mw       = rate_mw
    ...
    def _apps_update_loop(self):
        """Background thread: refreshes process list every 2 s while on apps page."""
        while True:
            if self.page != "apps":
                return
            try:
